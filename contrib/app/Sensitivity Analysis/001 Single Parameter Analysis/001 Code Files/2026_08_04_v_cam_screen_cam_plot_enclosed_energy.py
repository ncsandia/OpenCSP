...
def plot_enclosed_energy_curve_3(df, default_data_key='0b00'):
...
# def plot_energy_value_diff_6(df):

#     def energy_value_diff_plot_6(df, colorgrading='twilight_shifted',
#                                 plot_x_label= 'Reciever Widths (m)',
#                                 plot_y_label= 'Difference in Enclosed Energy (fraction)',
#                                 plot_title= 'Difference in Enclosed Energy as a Function of Reciever Widths'):
#         plt.figure(figsize=(10, 6))
...
def plot_energy_value_diff_7(df, output_dir, colorgrading='Spectral', save_plot=False):
    ...
    if isinstance(df, list):
        for i, single_df in enumerate(df):
            if 'vector_direction' in single_df.columns:
                ...
                imagefile_name_vectorized = fr'enclosed_energy_diff_plot_minimal_${{{vector_dir[0]}}}$'
            df_with_diff = add_energy_value_diff_column(single_df)
...
plot_energy_value_diff_7(dfs, analysis_folder, save_plot=True)
...
def plot_increment_vs_energy_diff(df, output_dir, save_plot=False, imagefile_name=None):
    ...
    if isinstance(df, list):
        for i, single_df in enumerate(df):
            if 'vector_direction' in single_df.columns:
                ...
                imagefile_name_vectorized = fr'enclosed_energy_and_increment_diff_plot_minimal${{{vector_dir[0]}}}$'
            df_with_diff = add_energy_value_diff_column(single_df)
...
plot_increment_vs_energy_diff(dfs, analysis_folder)
...
plot_increment_vs_energy_diff_2(dfs)
