...
def focal_length_line_plot(
    ...
    def plot_focal_length(
        x, y, legend_y, xlabel, ylabel, title, imagefile_name=None, y2=None, legend_y2=None, save_plot=save_plot
    ):
        fig, ax = plt.subplots()
        ...
        plt.tight_layout()
        if save_plot == True:
            image_file_name = 'focal_length_plot' if imagefile_name is None else imagefile_name
    ...
    def process_focal_length_plot(
        single_df, variable_compared, default_data, xlabel, imagefile_name=None, plot_title_assigned=plot_title_assigned
    ):
    ...
    if isinstance(df, list):
        ...
        process_focal_length_plot(df, variable_compared, default_data=default_data, xlabel=xlabel)
        ...
        if isinstance(df, list):
            for i, single_df in enumerate(df):
                if 'vector_direction' in single_df.columns:
                    vector_dir = single_df['vector_direction'].dropna().unique()
                if save_data:
                    csv_file_name = f'focal_length_comparison_subset_{vector_dir[0]}.csv'
