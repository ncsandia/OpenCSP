...
def process_hdf5_files(file_path_list, keyword, name_start, name_end=None, compare_to_baseline=True):
    ...
    if 'row' in rmse_df.columns:
        ...
        if compare_to_baseline:
            grouped_df = rmse_df.groupby('row') if 'row' in rmse_df.columns else None
            if grouped_df is not None:
                for row_value, group in rmse.
                default_rmse_series = grouped_df.loc[grouped_df['settings'] == '0b00', 'RMSE']
                default_rmse = default_rmse_series.iloc[0]
                diff = grouped_df['RMSE'] - default_rmse
                if len(diff) == len(rmse_df):
                    rmse_df['rmse_diff'] = None
                    rmse_df.loc[grouped_df.index, 'rmse_diff'] = diff
    else:
        if compare_to_baseline:
            default_rmse_series = rmse_df.loc[rmse_df['settings'] == '0b00', 'RMSE']
            default_rmse = default_rmse_series.iloc[0]
            rmse_df['rmse_diff'] = rmse_df['RMSE'] - default_rmse

    return rmse_df

df = process_hdf5_files(files = files, keyword = 'image_x', name_start = 'sa_',)
print(df)
...
def compute_rmse_difference(df, default_setting='0b00'):
    # Check if 'row' column exists
    group_col = 'row' if 'row' in df.columns else None
    ...
    if group_col:
        ...
        # Find RMSE for default setting in this group
            default_rmse_series = group_df.loc[group_df['settings'] == default_setting, 'RMSE']

            if default_rmse_series.empty:
                # Default setting not found in this group, skip or set diff as NaN
                print(f"Warning: Default setting '{default_setting}' not found in group '{group_key}'.")
                continue

            default_rmse = default_rmse_series.iloc[0]

            # Calculate difference for all rows in group
            diff = group_df['RMSE'] - default_rmse

            # Assign back to original DataFrame
            df.loc[group_df.index, 'rmse_diff'] = diff

    else:
        default_rmse_series = df.loc[df['settings'] == default_setting, 'RMSE']
        default_rmse = default_rmse_series.iloc[0]
        default_rmse = default_rmse_series.iloc[0]
        diff = group_df['RMSE'] - default_rmse
        df.loc[group_df.index, 'rmse_diff'] = diff
...
# Example usage:
# df = your DataFrame as shown
df = compute_rmse_difference(df, default_setting='0b00')
print(df)
...
sns.stripplot(x='increment', y='RMSE', data=df2, ax=axes[1, 1], jitter=True, palette='coolwarm', edgecolor='gray', size=5)
...
sns.stripplot(x='vector_direction', y='RMSE', data=df2, ax=axes[2, 1], jitter=True, palette='pastel', edgecolor='gray', size=5)
